...
namespace boundary
{
  ...
  public:
    ...
    bool outOfBounds(Position const& position) const
    {
      double radial_pos_sq = std::inner_product(position.cbegin()+1, position.cend(),
                                                position.cbegin()+1, 0.);
    ...
    bool operator()(State& state, State const& state_old = {}) const
    {
      ...
      std::vector<double> position_radial(2);
      std::vector<double> jump_radial(2);
      std::vector<double> tangent_at_contact(2);
      
      for (std::size_t dd = 0; dd < position_radial.size(); ++dd)
      {
        jump_radial[dd] = state.position[dd+1]-state_old.position[dd+1];
        position_radial[dd] = state.position[dd+1];
      }

      // Iterate reflection procedure until position is inside boundary
      double pos_sq = std::inner_product(position_radial.cbegin(), position_radial.cend(),
                                         position_radial.cbegin(), 0.);
      
      while (pos_sq > radius_sq)
      {
        // Fraction alpha of jump outside of boundary
        double pos_dot_jump = std::inner_product(position_radial.cbegin(), position_radial.cend(),
        ...
        double alpha = aux - std::sqrt(aux*aux - (pos_sq-radius*radius)/norm_sq_jump);

        // Place position at contact
        for (std::size_t dd = 0; dd < position_radial.size(); ++dd)
          position_radial[dd] -= alpha * jump_radial[dd];

        // Tangent to boundary at contact
        
        tangent_at_contact[0] = position_radial[1]/radius;
        tangent_at_contact[1] = -position_radial[0]/radius;
        ...
        double tangent_dot_jump = std::inner_product(tangent_at_contact.cbegin(), tangent_at_contact.cend(),
                                                     jump_radial.cbegin(), 0.);
        double cos_contact_angle = std::abs(tangent_dot_jump)/std::sqrt(norm_sq_jump);
        double contact_angle = std::acos(cos_contact_angle);

        // Angle between incoming and outgoing jump at collision
        double phi = constants::pi - 2.*contact_angle;
        double cosphi = std::cos(phi);
        double sinphi = std::sin(phi);

        // Outgoing jump at collision, rotate outer jump by -phi and flip the direction
        double jump_0 = -alpha*(cosphi*jump_radial[0] + sinphi*jump_radial[1]);
        double jump_1 = -alpha*(-sinphi*jump_radial[0] + cosphi*jump_radial[1]);
        jump_radial[0] = jump_0;
        jump_radial[1] = jump_1;

        // Place position at reflected point
        // If outside boundary, will compute jump again starting from the current contact
        for (std::size_t dd = 0; dd < position_radial.size(); ++dd)
          position_radial[dd] += jump_radial[dd];
        pos_sq = std::inner_product(position_radial.cbegin(), position_radial.cend(),
                                    position_radial.cbegin(), 0.);
      }

      // Place particle at reflected position
      for (std::size_t dd = 0; dd < position_radial.size(); ++dd)
        state.position[dd+1] = position_radial[dd];
