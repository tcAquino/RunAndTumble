...
namespace ctrw
{
  ...
  public:
    ...
    void operator()
    (double time, CTRW const& ctrw, OutputState output_state)
    {
  ...
  public:
    ...
    Measurer_collection(Measurer_collection const&) = delete;
    ...
    void operator()(Subject const& subject, Value tag)
    {
      ...
      for (std::size_t ii = 0; ii < subject.size(); ++ii)
      {
        output << delimiter << subject.size(ii);
        for (auto it = subject.cbegin(ii); it != subject.cend(ii); ++it)
          output << delimiter << *it;
      }
      output << "\n";
  ...
  public:
    ...
    Measurer_container(Measurer_container const&) = delete;
    ...
    void operator()(Subject const& subject, Getter get, Value tag)
    {
      ...
      {
        auto values = get(part);
        useful::print(output, values, 1, delimiter);
      }
      output << "\n";
    }
  ...
  public:
    ...
    Measurer_value(Measurer_value const&) = delete;
  ...
  {
  public:
    Measurer_crossing_dist(std::valarray<double> measure_at, std::size_t nr_particles = 0)
  ...
  {
  public:
    Measurer_dist(std::valarray<double> measure_at, std::size_t nr_particles = 0)
    : measure_at(measure_at)
    , measure_value(measure_at.size())
    {
  ...
  {
  public:
    Measurer_crossing_total(std::valarray<double> measure_at)
    : measure_at(measure_at)
    , measure_value(measure_at.size())
    {}
  ...
  {
  public:
    Measurer_mean_variance(std::valarray<double> measure_at)
    : measure_at(measure_at)
    , mean(measure_at.size())
